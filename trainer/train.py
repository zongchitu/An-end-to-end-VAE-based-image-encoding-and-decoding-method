...
vgg_model = models.vgg16(pretrained=True).features.eval().to("cuda:0")
...
def train(
    ...
    writer = SummaryWriter(log_dir="log/111")
        ...
        for step, (x, y) in enumerate(loader):
            ...
            if vs:
                ...
                loss = compute_total_loss_standard(
                    model_output=x_recon, model_input=x, vgg_model=vgg_model
                )
            loss.backward()
            optimizer.step()
            # 记录训练过程
